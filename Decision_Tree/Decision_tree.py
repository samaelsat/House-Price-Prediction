...
leakage_features = ['price', 'id', 'url', 'Cid', 'DOM','communityAverage', 'tradeTime','constructionTime']
...
tree. plot_tree(dt_best,
               ...
               filled=True,
               rounded=True,
               max_depth=3,  # Limited for readability
               fontsize=9)
plt.title('Decision Tree Structure (Limited to 3 levels)',
          fontsize=18, fontweight='bold', pad=20)
plt.tight_layout()
plt.savefig('tree_structure.png', dpi=300, bbox_inches='tight')
print("\n✅ Plot saved: 'tree_structure.png'")
plt.show()

print(f"\nTree Statistics:")
print(f"  Total depth:        {dt_best.get_depth()}")
print(f"  Number of leaves:  {dt_best.get_n_leaves()}")
print(f"  Features used:     {np.sum(dt_best.feature_importances_ > 0)}/{len(X_train.columns)}")
